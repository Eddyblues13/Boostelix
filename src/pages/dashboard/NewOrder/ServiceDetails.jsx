...
const ServiceDetails = ({ selectedService, selectedCategory, getPlatformIcon, metrics, isMobile = false }) => {
  return (
    <div className={`bg-white ${isMobile ? 'rounded-lg sm:rounded-xl p-3 sm:p-4' : 'rounded-2xl p-6'} shadow-sm border border-gray-100 ${!isMobile && 'sticky top-6'}`}>
      <h3 className={`${isMobile ? 'text-base sm:text-lg' : 'text-2xl'} font-semibold text-gray-800 ${isMobile ? 'mb-3 sm:mb-4' : 'mb-6'}`}>
        Service Details
      </h3>
      
      {/* Service Header */}
      <div className={`bg-blue-600 rounded-lg sm:rounded-xl ${isMobile ? 'p-3 sm:p-4' : 'p-4'} text-white ${isMobile ? 'mb-3 sm:mb-4' : 'mb-6'}`}>
      ...
      {/* Service Metrics */}
      <div className={`grid grid-cols-2 ${isMobile ? 'gap-2 sm:gap-3 mb-3 sm:mb-4' : 'gap-4 mb-6'}`}>
        <div className={`bg-gray-50 rounded-lg sm:rounded-xl ${isMobile ? 'p-2.5 sm:p-3' : 'p-4'}`}>
          <h5 className={`${isMobile ? 'text-[10px] sm:text-xs' : 'text-sm'} font-medium text-gray-600 ${isMobile ? 'mb-1.5' : 'mb-2'}`}>
            Start Time
          </h5>
          <p className={`${isMobile ? 'text-xs sm:text-sm' : 'text-base'} text-gray-800`}>
            {metrics?.startTime || "--"}
          </p>
        </div>
        ...
        <div className={`bg-gray-50 rounded-lg sm:rounded-xl ${isMobile ? 'p-2.5 sm:p-3' : 'p-4'}`}>
          ...
          <p className={`${isMobile ? 'text-xs sm:text-sm' : 'text-base'} text-gray-800 font-medium`}>
            {metrics?.avgTime || "--"}
          </p>
        </div>
      ...
      {/* Service Description */}
      <div>
        <h5 className={`${isMobile ? 'text-sm sm:text-base' : 'text-lg'} font-medium text-gray-600 ${isMobile ? 'mb-2 sm:mb-3' : 'mb-4'}`}>
          Description
        </h5>
        <div className={`${isMobile ? 'text-xs sm:text-sm' : 'text-base'} text-gray-700 ${isMobile ? 'space-y-1.5 sm:space-y-2' : 'space-y-3'}`}>
